...
class YOLODetect(object):
    ...
    def __init__(self,base_path,weights,config,input_size=416 ):
        ...
        self.net = cv2.dnn.readNetFromONNX(self.weightsPath)    #使用opencv读取权重文件
        if self.cuda:  #如果支持GPU，则启用opencv的GPU支持
            self.net.setPreferableBackend(cv2.dnn.DNN_BACKEND_CUDA)
            self.net.setPreferableTarget(cv2.dnn.DNN_TARGET_CUDA)
        else:  #否则使用CPU运算
            self.net.setPreferableBackend(cv2.dnn.DNN_BACKEND_OPENCV)
            self.net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)
        self.layers = self.net.getLayerNames() #取得所定义的层的名称，一般为'boxes', 'confs'，'??'
        self.layers = [self.layers[i - 1] for i in self.net.getUnconnectedOutLayers()]
        self.layers = self.layers[::-1]  #列表顺序与其它刚好相反，这里反转列表，使得与其他方法顺序一致
    ...
    def get_processed_image(self,image, atype, show_fps=False, confThreshold=0.5, nmsThreshold=0.4):
        ...
        #调整图片尺寸，并归一化为小数
        blob = cv2.dnn.blobFromImage(image, 1 / 255.0, self.input_shape, crop=False) 

        # Run a model
        self.net.setInput(blob)  #开始检测
